...
class Categoria {
	...
	//Metodo que sirve para realizar una busqueda de una categoria por nombre
	public function getCategoriaFiltrado($busqueda){
		
		$this->getConection();
		$sql = "SELECT * FROM ".$this->table." WHERE nombreCategoria LIKE "."'".$busqueda."'";
		$result = $this->conection->query($sql);
	...
	public function save($param){
		...
		if(isset($param["id"])  and $param["id"] !=''){
			$actualCategoria = $this->getCategoriaById($param["id"]);
			if(isset($actualCategoria["idCategoria"])){
				$exists = true;	
				/* valores que quiero modificar*/
				$id = $param["id"];
				$nombre = $actualCategoria["nombreCategoria"];
			}
		}
		...
		/* SQL*/
			...
			if($exists){ 
				$sql = "UPDATE ".$this->table." SET nombreCategoria=? WHERE idCategoria=?";
				$stmt = $this->conection->prepare($sql);
